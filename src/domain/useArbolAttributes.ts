...
export function useArbolAttributes(selectedNodes: ComputedRef<ArbolNode[]>, nivel: ComputedRef<number> = computed(() => 1)) {
  ...
  function getAttributeType(attributeId: number): number {
    const atributo = atributosData.caracteristicasSecundarias.find(a => a.id === attributeId)
  ...
  function calculateAttributeWithMultiplier(attributeId: number, base: number = 0, factor1: number = 3,nivel: number = 0, factor2: number = factor1): number {
  ...
  // Atributos que suman 3 por nodo + 3 por atributo principal según su tipo
  const hp = computed(() => calculateAttributeWithMultiplier(7,10,2*nivel.value,1,3)) // ID 7 = HP (base = nivel, tipo 1 = Cuerpo)
  ...
  const regeneracion = computed(() => calculateAttributeWithMultiplier(8,2,1)) // ID 8 = Regeneración (tipo 1 = Cuerpo)
