...
export function realizarCarga(
  ...
  hexagonosDisponibles.forEach((hex) => {
    ...
    const productoEscalar =
      dirHaciaObjetivoX * direccionX + dirHaciaObjetivoY * direccionY;
...
export function realizarEmpujar(
  ...
  if (dado1 === 1 || dado2 === 1) {
    return {
      ...
      acerto: false,
      mensaje:
        `${atacante.nombre} intenta empujar a ${defensor.nombre}!\n` +
        `🎲 Tirada: ${dado1} + ${dado2}\n` +
  ...
  const distanciaEmpuje = Math.max(
    0,
    Math.floor((poderioAtacante - poderioDefensor) / 3),
  );
  ...
  if (distanciaEmpuje === 0) {
    return {
      ...
      acerto: true,
      mensaje:
        `${atacante.nombre} empuja a ${defensor.nombre}!\n` +
  ...
  for (let dist = 1; dist <= distanciaEmpuje; dist++) {
    ...
    const hexDisponible = hexagonosDisponibles.find(
 ...
 */
export function calcularIniciativas(
...
export interface ResultadoReaccion {
  ...
  ataqueContra?: {
    // Para Contraataque y Ataque de Oportunidad
    tirada: number;
    dado1: number;
    dado2: number;
    danio: number;
...
export function realizarContraataque(
  ...
  if (dado1 === 1 || dado2 === 1) {
    return {
      ...
      exito: false,
      mensaje:
        `⚔️ ${defensor.nombre} intenta un Contraataque!\n` +
  ...
  // Verificar crítico
  const rangoCriticoArma = armaDefensor?.rango_critico
    ? parseInt(armaDefensor.rango_critico)
    : 999;
  ...
  const defensaPenetrante = Math.max(
    0,
    defensaAtacanteOriginal.penetrante - punteria,
  );
  const defensaLacerante = Math.max(
    0,
    defensaAtacanteOriginal.lacerante - punteria,
  );
  const defensaContundente = Math.max(
    0,
    defensaAtacanteOriginal.contundente - punteria,
  );
  ...
  const mensaje =
    ...
    (esCritico
      ? `⭐ ¡CRÍTICO! ${armaDefensor?.critico || "x2"} (Rango: ${rangoCriticoFinal})\n`
      : "") +
    `🎯 Puntería: ${punteria}\n` +
 ...
 */
export function realizarTension(
  defensor: PersonajeInstancia,
): ResultadoReaccion {
  const modificador = 3;
...
export function realizarInterceptar(
  ...
  return {
    ...
    exito: true,
    mensaje:
      `🛡️ ${interceptor.nombre} intercepta el ataque!\n` +
...
export function realizarAtaqueOportunidad(
  ...
  if (dado1 === 1 || dado2 === 1) {
    return {
      ...
      exito: false,
      mensaje:
        `⚡ ${atacante.nombre} intenta un Ataque de Oportunidad!\n` +
        `🎲 Tirada: ${dado1} + ${dado2} + ${atacante.nivel} = ${tirada}\n` +
        `💥 ¡FALLO CRÍTICO! (dado = 1)\n` +
  ...
  const rangoCriticoArma = armaAtacante?.rango_critico
    ? parseInt(armaAtacante.rango_critico)
    : 999;
  ...
  const defensaContundente = Math.max(
    0,
    defensaObjetivo.contundente - punteria,
  );
  ...
  const mensaje =
    ...
    (esCritico
      ? `⭐ ¡CRÍTICO! ${armaAtacante?.critico || "x2"} (Rango: ${rangoCriticoFinal})\n`
      : "") +
    `🎯 Puntería: ${punteria}\n` +
    `💥 Daño: P:${danioFinalPenetrante} L:${danioFinalLacerante} C:${danioFinalContundente} = ${danioTotal} total`;
