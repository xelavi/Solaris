<template>
  <div class="w-full h-screen relative">
    ...
    <!-- Turn Info / Actions -->
    <div class="absolute bottom-4 right-4 text-white bg-black/50 p-4 rounded pointer-events-none">
        <p v-if="personajeActivo">Turno de: <span class="font-bold text-yellow-400">{{ personajeActivo.nombre }}</span></p>
...
const panelData = ref<{ nombre: string; vidaActual: number; vidaMax: number } | null>(null);
...
watch(partidaActual, (newPartida) => {
    if (newPartida) {
...
watch(personajeSeleccionado, (pj) => {
    ...
    if (pj && personajeActivo.value && pj.nombre === personajeActivo.value.nombre) {
...
function animate() {
    ...
    characterMeshes.forEach(mesh => {
        if (mesh.userData.isMoving && mesh.userData.pathQueue && mesh.userData.pathQueue.length > 0) {
            ...
            if (personajeActivo.value && mesh.userData.characterData.nombre === personajeActivo.value.nombre && circleMesh) {
...
async function onCanvasClick(event: MouseEvent) {
    ...
    if (intersects.length > 0) {
        ...
        if (obj.userData.isCharacter) {
            ...
            personajeSeleccionado.value = obj.userData.characterData as PersonajeInstancia;
            ...
            if (
                ...
                if (result.exito && result.camino) {
                    ...
                    if (mesh) {
                        ...
                        mesh.userData.pathQueue = result.camino.map((c: any) => new THREE.Vector3(c.x, 1, c.z));
                        // Remove first point if it's current position
                        if (mesh.userData.pathQueue.length > 0 && mesh.userData.pathQueue[0].distanceTo(mesh.position) < 0.1) {
                            mesh.userData.pathQueue.shift();
                        }
                    }
