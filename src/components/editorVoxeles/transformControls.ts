...
export class TransformControls extends Object3D {
	...
	private _getPointer: (event: MouseEvent) => { x: number, y: number, button: number };
...
class TransformControlsGizmo extends Object3D {
	...
	constructor(controls: TransformControls) {
		...
		const gizmoMaterial = new MeshBasicMaterial( { depthTest: false, depthWrite: false, fog: false, toneMapped: false, transparent: true } );
		const gizmoLineMaterial = new LineBasicMaterial( { depthTest: false, depthWrite: false, fog: false, toneMapped: false, transparent: true } );
		...
		const matRed = gizmoMaterial.clone(); matRed.color.setHex( 0xff0000 );
		const matGreen = gizmoMaterial.clone(); matGreen.color.setHex( 0x00ff00 );
		const matBlue = gizmoMaterial.clone(); matBlue.color.setHex( 0x0000ff );
		const matRedTransparent = gizmoMaterial.clone(); matRedTransparent.color.setHex( 0xff0000 ); matRedTransparent.opacity = 0.5;
		const matGreenTransparent = gizmoMaterial.clone(); matGreenTransparent.color.setHex( 0x00ff00 ); matGreenTransparent.opacity = 0.5;
		const matBlueTransparent = gizmoMaterial.clone(); matBlueTransparent.color.setHex( 0x0000ff ); matBlueTransparent.opacity = 0.5;
		const matWhiteTransparent = gizmoMaterial.clone(); matWhiteTransparent.opacity = 0.25;
		const matYellowTransparent = gizmoMaterial.clone(); matYellowTransparent.color.setHex( 0xffff00 ); matYellowTransparent.opacity = 0.25;
		const matYellow = gizmoMaterial.clone(); matYellow.color.setHex( 0xffff00 );
		...
		this.materialLib = {
			...
			xAxisTransparent: matRedTransparent, yAxisTransparent: matGreenTransparent, zAxisTransparent: matBlueTransparent, activeTransparent: matYellowTransparent
		};

		const arrowGeometry = new CylinderGeometry( 0, 0.04, 0.1, 12 ); arrowGeometry.translate( 0, 0.05, 0 );
		const scaleHandleGeometry = new BoxGeometry( 0.08, 0.08, 0.08 ); scaleHandleGeometry.translate( 0, 0.04, 0 );
		const lineGeometry = new BufferGeometry(); lineGeometry.setAttribute( 'position', new Float32BufferAttribute( [ 0, 0, 0,	1, 0, 0 ], 3 ) );
		const lineGeometry2 = new CylinderGeometry( 0.0075, 0.0075, 0.5, 3 ); lineGeometry2.translate( 0, 0.25, 0 );

		function CircleGeometry( radius: number, arc: number ) {
			const geometry = new TorusGeometry( radius, 0.0075, 3, 64, arc * Math.PI * 2 );
			geometry.rotateY( Math.PI / 2 ); geometry.rotateX( Math.PI / 2 ); return geometry;
		}

		function TranslateHelperGeometry() {
			const geometry = new BufferGeometry(); geometry.setAttribute( 'position', new Float32BufferAttribute( [ 0, 0, 0, 1, 1, 1 ], 3 ) ); return geometry;
		}
		...
		const pickerTranslate: any = {
			X: [ [ new Mesh( new CylinderGeometry( 0.4, 0, 0.6, 4 ), matInvisible ), [ 0.3, 0, 0 ], [ 0, 0, - Math.PI / 2 ]], [ new Mesh( new CylinderGeometry( 0.4, 0, 0.6, 4 ), matInvisible ), [ - 0.3, 0, 0 ], [ 0, 0, Math.PI / 2 ]] ],
			Y: [ [ new Mesh( new CylinderGeometry( 0.4, 0, 0.6, 4 ), matInvisible ), [ 0, 0.3, 0 ]], [ new Mesh( new CylinderGeometry( 0.4, 0, 0.6, 4 ), matInvisible ), [ 0, - 0.3, 0 ], [ 0, 0, Math.PI ]] ],
			Z: [ [ new Mesh( new CylinderGeometry( 0.4, 0, 0.6, 4 ), matInvisible ), [ 0, 0, 0.3 ], [ Math.PI / 2, 0, 0 ]], [ new Mesh( new CylinderGeometry( 0.4, 0, 0.6, 4 ), matInvisible ), [ 0, 0, - 0.3 ], [ - Math.PI / 2, 0, 0 ]] ],
			XYZ: [ [ new Mesh( new OctahedronGeometry( 0.4, 0 ), matInvisible ) ] ],
			XY: [ [ new Mesh( new BoxGeometry( 0.4, 0.4, 0.01 ), matInvisible ), [ 0.15, 0.15, 0 ]] ],
			YZ: [ [ new Mesh( new BoxGeometry( 0.4, 0.4, 0.01 ), matInvisible ), [ 0, 0.15, 0.15 ], [ 0, Math.PI / 2, 0 ]] ],
			XZ: [ [ new Mesh( new BoxGeometry( 0.4, 0.4, 0.01 ), matInvisible ), [ 0.15, 0, 0.15 ], [ - Math.PI / 2, 0, 0 ]] ]
		};
		...
		const helperRotate: any = { AXIS: [ [ new Line( lineGeometry, matHelper ), [ - 1e3, 0, 0 ], null, [ 1e6, 1, 1 ], 'helper' ] ] };

		const pickerRotate: any = {
			XYZE: [ [ new Mesh( new SphereGeometry( 0.25, 10, 8 ), matInvisible ) ] ],
			X: [ [ new Mesh( new TorusGeometry( 0.5, 0.25, 4, 24 ), matInvisible ), [ 0, 0, 0 ], [ 0, - Math.PI / 2, - Math.PI / 2 ]], ],
			Y: [ [ new Mesh( new TorusGeometry( 0.5, 0.25, 4, 24 ), matInvisible ), [ 0, 0, 0 ], [ Math.PI / 2, 0, 0 ]], ],
			Z: [ [ new Mesh( new TorusGeometry( 0.5, 0.25, 4, 24 ), matInvisible ), [ 0, 0, 0 ], [ 0, 0, - Math.PI / 2 ]], ],
		...
		function setupGizmo( gizmoMap: any ) {
			...
			for ( const name in gizmoMap ) {
				for ( let i = gizmoMap[ name ].length; i --; ) {
					...
					object.name = name; object.userData.tag = tag;
	...
	updateMatrixWorld( force?: boolean ) {
		...
		const quaternion = ( space === 'local' ) ? this.controls.worldQuaternion : _identityQuaternion;
		...
		for ( let i = 0; i < handles.length; i ++ ) {
			...
			if ( this.controls.camera.isOrthographicCamera ) {
				// @ts-ignore
				factor = ( this.controls.camera.top - this.controls.camera.bottom ) / this.controls.camera.zoom;
			} else {
				// @ts-ignore
				factor = worldPosition.distanceTo( this.controls.cameraPosition ) * Math.min( 1.9 * Math.tan( Math.PI * this.controls.camera.fov / 360 ) / this.controls.camera.zoom, 7 );
			}
			...
			if ( handle.userData.tag === 'helper' ) {
				...
				if ( handle.name === 'AXIS' ) {
					...
					if ( axis === 'X' ) {
						...
						if ( Math.abs( _alignVector.copy( _unitX ).applyQuaternion( quaternion ).dot( eye ) ) > 0.9 ) handle.visible = false;
					}
					if ( axis === 'Y' ) {
						_tempQuaternion.setFromEuler( _tempEuler.set( 0, 0, Math.PI / 2 ) );
						handle.quaternion.copy( quaternion ).multiply( _tempQuaternion );
						if ( Math.abs( _alignVector.copy( _unitY ).applyQuaternion( quaternion ).dot( eye ) ) > 0.9 ) handle.visible = false;
					}
					if ( axis === 'Z' ) {
						_tempQuaternion.setFromEuler( _tempEuler.set( 0, Math.PI / 2, 0 ) );
						handle.quaternion.copy( quaternion ).multiply( _tempQuaternion );
						if ( Math.abs( _alignVector.copy( _unitZ ).applyQuaternion( quaternion ).dot( eye ) ) > 0.9 ) handle.visible = false;
					}
					if ( axis === 'XYZE' ) {
						_tempQuaternion.setFromEuler( _tempEuler.set( 0, Math.PI / 2, 0 ) );
					...
					handle.position.copy( this.controls.worldPositionStart ); handle.visible = this.controls.dragging;
				} else if ( handle.name === 'END' ) {
					handle.position.copy( worldPosition ); handle.visible = this.controls.dragging;
				} else if ( handle.name === 'DELTA' ) {
					handle.position.copy( this.controls.worldPositionStart ); handle.quaternion.copy( this.controls.worldQuaternionStart );
					_tempVector.set( 1e-10, 1e-10, 1e-10 ).add( this.controls.worldPositionStart ).sub( worldPosition ).multiplyScalar( - 1 );
					_tempVector.applyQuaternion( this.controls.worldQuaternionStart.clone().invert() );
					handle.scale.copy( _tempVector ); handle.visible = this.controls.dragging;
				} else {
					handle.quaternion.copy( quaternion );
					if ( this.controls.dragging ) handle.position.copy( this.controls.worldPositionStart );
			...
			if ( mode === 'translate' || mode === 'scale' ) {
				const AXIS_HIDE_THRESHOLD = 0.99; const PLANE_HIDE_THRESHOLD = 0.2;
				if ( handle.name === 'X' && Math.abs( _alignVector.copy( _unitX ).applyQuaternion( quaternion ).dot( eye ) ) > AXIS_HIDE_THRESHOLD ) { handle.scale.set( 1e-10, 1e-10, 1e-10 ); handle.visible = false; }
				if ( handle.name === 'Y' && Math.abs( _alignVector.copy( _unitY ).applyQuaternion( quaternion ).dot( eye ) ) > AXIS_HIDE_THRESHOLD ) { handle.scale.set( 1e-10, 1e-10, 1e-10 ); handle.visible = false; }
				if ( handle.name === 'Z' && Math.abs( _alignVector.copy( _unitZ ).applyQuaternion( quaternion ).dot( eye ) ) > AXIS_HIDE_THRESHOLD ) { handle.scale.set( 1e-10, 1e-10, 1e-10 ); handle.visible = false; }
				if ( handle.name === 'XY' && Math.abs( _alignVector.copy( _unitZ ).applyQuaternion( quaternion ).dot( eye ) ) < PLANE_HIDE_THRESHOLD ) { handle.scale.set( 1e-10, 1e-10, 1e-10 ); handle.visible = false; }
				if ( handle.name === 'YZ' && Math.abs( _alignVector.copy( _unitX ).applyQuaternion( quaternion ).dot( eye ) ) < PLANE_HIDE_THRESHOLD ) { handle.scale.set( 1e-10, 1e-10, 1e-10 ); handle.visible = false; }
				if ( handle.name === 'XZ' && Math.abs( _alignVector.copy( _unitY ).applyQuaternion( quaternion ).dot( eye ) ) < PLANE_HIDE_THRESHOLD ) { handle.scale.set( 1e-10, 1e-10, 1e-10 ); handle.visible = false; }
			} else if ( mode === 'rotate' ) {
				_tempQuaternion2.copy( quaternion );
				_alignVector.copy( eye ).applyQuaternion( _tempQuaternion.copy( quaternion ).invert() );
				if ( handle.name.search( 'E' ) !== - 1 ) handle.quaternion.setFromRotationMatrix( _lookAtMatrix.lookAt( eye, _zeroVector, _unitY ) );
				if ( handle.name === 'X' ) { _tempQuaternion.setFromAxisAngle( _unitX, Math.atan2( - _alignVector.y, _alignVector.z ) ); _tempQuaternion.multiplyQuaternions( _tempQuaternion2, _tempQuaternion ); handle.quaternion.copy( _tempQuaternion ); }
				if ( handle.name === 'Y' ) { _tempQuaternion.setFromAxisAngle( _unitY, Math.atan2( _alignVector.x, _alignVector.z ) ); _tempQuaternion.multiplyQuaternions( _tempQuaternion2, _tempQuaternion ); handle.quaternion.copy( _tempQuaternion ); }
				if ( handle.name === 'Z' ) { _tempQuaternion.setFromAxisAngle( _unitZ, Math.atan2( _alignVector.y, _alignVector.x ) ); _tempQuaternion.multiplyQuaternions( _tempQuaternion2, _tempQuaternion ); handle.quaternion.copy( _tempQuaternion ); }
			}
			...
			if ( this.controls.enabled && axis ) {
				if ( handle.name === axis ) { handle.material.color.copy( this.materialLib.active.color ); handle.material.opacity = 1.0; }
				else if ( axis.split( '' ).some( function ( a: string ) { return handle.name === a; } ) ) { handle.material.color.copy( this.materialLib.active.color ); handle.material.opacity = 1.0; }
			}
		}
		super.updateMatrixWorld( force );
	}
...
class TransformControlsPlane extends Mesh {
	...
	updateMatrixWorld( force?: boolean ) {
		...
		switch ( mode ) {
			...
			case 'scale':
				switch ( axis ) {
					case 'X': _alignVector.copy( eye ).cross( _v1 ); _dirVector.copy( _v1 ).cross( _alignVector ); break;
					case 'Y': _alignVector.copy( eye ).cross( _v2 ); _dirVector.copy( _v2 ).cross( _alignVector ); break;
					case 'Z': _alignVector.copy( eye ).cross( _v3 ); _dirVector.copy( _v3 ).cross( _alignVector ); break;
					...
					case 'XYZ': case 'E': _dirVector.set( 0, 0, 0 ); break;
				}
				break;
			case 'rotate': default: _dirVector.set( 0, 0, 0 );
		}
		if ( _dirVector.length() === 0 ) { this.quaternion.copy( this.controls.cameraQuaternion ); }
		else { _tempMatrix.lookAt( _tempVector.set( 0, 0, 0 ), _dirVector, _alignVector ); this.quaternion.setFromRotationMatrix( _tempMatrix ); }
		super.updateMatrixWorld( force );
	}
}