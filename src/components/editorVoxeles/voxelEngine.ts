...
import type { VoxelData, VoxelMaterial, BrushSettings, PaletteItem, EngineCallbacks, HistoryAction, ToolMode, Symmetries } from './types';
...
export class VoxelEngine {
  ...
  private init() {
    ...
    this.orthoCamera = new THREE.OrthographicCamera(-1000, 1000, 1000, -1000, 1, 10000);
    ...
    const renderTarget = new THREE.WebGLRenderTarget(window.innerWidth, window.innerHeight, { type: THREE.HalfFloatType, format: THREE.RGBAFormat });
    ...
    window.addEventListener('keydown', (e) => { 
        ...
        if(e.key === 'Enter' && this.currentTool === 'select') this.clearSelection(false);
    });
  ...
  private initToolsHelpers() {
    ...
    const mat = new THREE.MeshBasicMaterial({ color: 0x44aaff, opacity: 0.4, transparent: true, depthTest: false });
    ...
    this.boxPreviewMesh = new THREE.Mesh(
        ...
        new THREE.MeshBasicMaterial({ color: 0x44aaff, opacity: 0.5, transparent: true, depthTest: false })
    );
  ...
  private createWorldObjects() {
    ...
    const floorMat = new THREE.ShaderMaterial({
        ...
        vertexShader: baseGridShader.vertexShader, fragmentShader: baseGridShader.fragmentShader,
    ...
    const cursorMat = new THREE.ShaderMaterial({
        uniforms: { uSize: { value: this.voxelSize }, uColor: { value: new THREE.Color(0x00ffff) }, uWorldOffset: { value: new THREE.Vector3() } },
        vertexShader: cursorGridShader.vertexShader, fragmentShader: cursorGridShader.fragmentShader,
        transparent: true, depthWrite: false, extensions: { derivatives: true }
    });
    this.cursorGrid = new THREE.Mesh(new THREE.PlaneGeometry(500, 500), cursorMat);
  ...
  public setTool(tool: ToolMode) {
    ...
    // Reset tools
    if (tool !== 'box') { this.boxStart = null; this.boxPreviewMesh.visible = false; }
  ...
  public setSymmetries(s: Symmetries) { this.symmetries = s; }
  public updateSettings(m: VoxelMaterial, b: BrushSettings, p: PaletteItem[]) {
    this.currentMaterial = m; this.brush = b; this.brushPalette = p;
  }
  ...
  private onPointerDown(event: MouseEvent) {
    ...
    if (this.currentTool === 'picker' || this.isAltDown) {
      const pickPos = gridPos.clone().sub(normal.clone().multiplyScalar(0.01)).floor();
      const mat = this.chunkManager.getVoxel(pickPos.x, pickPos.y, pickPos.z);
      if (mat && this.callbacks.onMaterialPick) this.callbacks.onMaterialPick(mat);
      return;
    }
  ...
  private onPointerMove(event: PointerEvent) {
    ...
    if (this.isPointerDown) {
        const dist = new THREE.Vector2(event.clientX, event.clientY).distanceTo(this.pointerStart);
    ...
    if (this.isPainting && this.currentTool === 'brush' && !this.isDraggingCamera && hit) {
    ...
    if (hit) {
        ...
        if (this.currentTool === 'brush' && !this.isTransforming && !this.isPainting) {
            this.updateBrushPreview(hit);
        }
        ...
        if (this.currentTool === 'box' && this.boxStart) {
             let target = this.isShiftDown 
                ...
                : hit.gridPos.clone().add(hit.normal.clone().multiplyScalar(0.01)).floor();
             const snap = this.gridSnap;
  ...
  private raycast(event: MouseEvent) {
    const objects = [this.plane, ...this.scene.children.filter(c => c.userData.isChunk)];
    ...
    for (let i = 0; i < intersects.length; i++) {
        ...
        if (hit.object === this.brushPreviewMesh || hit.object === this.boxPreviewMesh || hit.object === this.selectionPreviewBox) continue;
  ...
  private updateBrushPreview(hit: {gridPos: THREE.Vector3, normal: THREE.Vector3}) {
      ...
      if (this.symmetries.x) centers.push(new THREE.Vector3(-origin.x - snap, origin.y, origin.z));
      if (this.symmetries.z) {
          const len = centers.length;
          for(let i=0; i<len; i++) centers.push(new THREE.Vector3(centers[i].x, centers[i].y, -centers[i].z - snap));
      }
      ...
      // @ts-ignore
      this.brushPreviewMesh.material.color.setHex(this.isShiftDown ? 0xff0000 : 0x44aaff);
  }

  private calculateBrushStroke(hit: { gridPos: THREE.Vector3, normal: THREE.Vector3 }) {
                ...
                for(let sy=0; sy<snap; sy++) {
                    for(let sz=0; sz<snap; sz++) {
                         ...
                         if (!this.tempVoxels.has(key)) {
                             ...
                             if (this.isShiftDown) {
                                 if (existing) {
                                     this.tempVoxels.set(key, { ...existing, x: tx, y: ty, z: tz }); 
                                 }
                             } else {
  ...
  private updateStrokePreview() {
      ...
      if(this.strokePreviewMesh.instanceColor) this.strokePreviewMesh.instanceColor.needsUpdate = true;
  }
  ...
  private commitBrushStroke() {
      ...
      this.tempVoxels.forEach((v, key) => {
           ...
           if (this.isShiftDown) {
               ...
               if (existing) batchChanges.removed.push({ data: { ...existing, x, y, z } });
  ...
  private performVoxelOp(x: number, y: number, z: number, batch: HistoryAction['changes']) {
      const existing = this.chunkManager.getVoxel(x, y, z);
      if (this.isShiftDown) {
          if (existing) {
  ...
  private createSelection(start: THREE.Vector3, end: THREE.Vector3) {
      ...
      const mesh = new THREE.InstancedMesh(geo, new THREE.MeshStandardMaterial({ color: 0xffffff }), extracted.length);
      ...
      if(this.selectionGroup.children.length > 0) {
          ...
          if (cancel) {
              ...
              for(let i=0; i<original.length; i++) {
                  ...
                  this.chunkManager.setVoxel(gx, gy, gz, original[i]);
              }
          }
    ...
    for(let x=-r; x<=r; x++) {
        for(let y=0; y<=r*2; y++) {
            for(let z=-r; z<=r; z++) {
                 ...
                 else if (shape === 'sphere') { if (Math.sqrt(x*x + cy*cy + z*z) <= r + 0.5) include = true; }
  ...
  private recordAction(action: HistoryAction) {
    ...
    if (this.callbacks.onVoxelCountChange) this.callbacks.onVoxelCountChange(this.chunkManager.getVoxelCount());
  }
  ...
  public undo() {
    ...
    if (action.type === 'batch') {
      action.changes.added.forEach(item => { this.chunkManager.setVoxel(item.data.x, item.data.y, item.data.z, null); });
      action.changes.removed.forEach(item => { this.chunkManager.setVoxel(item.data.x, item.data.y, item.data.z, item.data); });
    }
    this.historyIndex--;
    if (this.callbacks.onHistoryChange) this.callbacks.onHistoryChange(this.historyIndex, this.history.length);
  }
  ...
  public redo() {
    ...
    const action = this.history[this.historyIndex];
    if (action.type === 'batch') {
      action.changes.removed.forEach(item => { this.chunkManager.setVoxel(item.data.x, item.data.y, item.data.z, null); });
      action.changes.added.forEach(item => { this.chunkManager.setVoxel(item.data.x, item.data.y, item.data.z, item.data); });
    }
    if (this.callbacks.onHistoryChange) this.callbacks.onHistoryChange(this.historyIndex, this.history.length);
  }

  public adjustLevel(d: number) { this.currentLevel += d; const y = this.currentLevel * this.voxelSize; this.plane.position.y = y; this.infiniteGrid.position.y = y + 0.1; }
  ...
  public toggleCamera(iso: boolean) {
      ...
      this.controls = new OrbitControls(this.camera, this.renderer.domElement);
      this.controls.enableDamping = true; this.controls.dampingFactor = 0.05; this.controls.target.copy(oldTarget);
      this.controls.mouseButtons = { LEFT: null, MIDDLE: THREE.MOUSE.PAN, RIGHT: THREE.MOUSE.ROTATE };
  ...
  public setDarkMode(d: boolean) { this.scene.background = new THREE.Color(d ? 0x1a1a1a : 0xf0f0f0); }
  public setModifiers(shift: boolean, alt: boolean) { if (this.isShiftDown !== shift || this.isAltDown !== alt) { this.isShiftDown = shift; this.isAltDown = alt; this.brushPreviewMesh.visible = false; } } 
  public takeScreenshot(cb: any) { this.renderScene(); cb(this.renderer.domElement.toDataURL('image/png')); }
  public clearAll() { this.chunkManager.clear(); this.history = []; this.historyIndex = -1; }
  ...
  public exportVoxels() {
    ...
    const exportData = { version: 1, voxels: voxels, gridSize: this.voxelSize, timestamp: new Date().toISOString() };
  ...
  private onWindowResize() {
    ...
    if (this.perspectiveCamera) { this.perspectiveCamera.aspect = w / h; this.perspectiveCamera.updateProjectionMatrix(); }
    if (this.orthoCamera) { const s = 1000; this.orthoCamera.left = -s * (w/h) / 2; this.orthoCamera.right = s * (w/h) / 2; this.orthoCamera.top = s / 2; this.orthoCamera.bottom = -s / 2; this.orthoCamera.updateProjectionMatrix(); }
    this.renderer.setSize(w, h);
    this.composer.setSize(w, h);
    this.updateFXAA();
  }
  
  private updateFXAA() { if (this.fxaaPass) { const pixelRatio = this.renderer.getPixelRatio(); this.fxaaPass.material.uniforms['resolution'].value.x = 1 / (window.innerWidth * pixelRatio); this.fxaaPass.material.uniforms['resolution'].value.y = 1 / (window.innerHeight * pixelRatio); } }
  ...
  private renderScene() { this.composer.render(); }
  public dispose() { this.controls.dispose(); this.renderer.dispose(); this.transformControls.dispose(); this.thumbRenderer?.dispose(); window.removeEventListener('resize', this.onWindowResize); }
}