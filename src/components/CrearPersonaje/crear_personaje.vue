    ...
    <div class="max-w-350 mx-auto">
      ...
      <div class="flex flex-col lg:flex-row gap-6">
        <div class="lg:w-64 flex-shrink-0">
          <div
            class="bg-white backdrop-blur-sm border-2 border-gray-200 rounded-xl p-4 shadow-xl"
          >
            <h2
              class="text-lg font-bold text-gray-700 mb-4 pb-3 border-b border-gray-200"
            >
              Índice
            </h2>
            <nav class="space-y-2">
        ...
        <div class="flex-1">
          <div
            class="bg-white backdrop-blur-sm border-2 border-gray-200 rounded-xl p-8 shadow-xl min-h-[500px]"
          >
            <h2
              class="text-3xl font-bold text-gray-600 mb-6 pb-4 border-b border-gray-200"
            >
              {{ steps[currentStep] }}
            </h2>
            ...
            >
              ...
              <div
                class="px-6 py-3 text-green-500 font-semibold flex items-center gap-2"
              >
                <span>✓</span>
...
function volverAPersonajes() {
  ...
  const id =
    personajeId.value || localStorage.getItem("personaje_en_creacion_id");

  // Si el personaje no tiene los datos esenciales, eliminarlo
  if (id && character) {
    const tieneNombre = character.name && character.name.trim() !== "";
    const tieneTrasfondo =
      character.trasfondo && character.trasfondo.trim() !== "";
    const tieneOficio = character.oficio && character.oficio.trim() !== "";
    const tieneEstiloMarcial =
      character.estilo_marcial && character.estilo_marcial.trim() !== "";
...
function verFicha() {
  ...
  if (!personajeGuardado) {
    alert(
      "⚠️ No hay personaje guardado. Por favor, guarda el personaje primero.",
    );
    return;
  }
...
function debugDatos() {
  ...
  console.log(
    "character keys:",
    character ? Object.keys(character) : "no character",
  );
...
function guardarPersonaje() {
  try {
    ...
    console.log(
      "💾 Guardando personaje con characterData:",
      characterData.value,
    );
    ...
    console.log(
      "✅ Datos preparados para guardar con ID:",
      id,
      datosParaGuardar,
    );
