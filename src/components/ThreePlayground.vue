...
function init() {
  ...
  for (let j = 0; j < gridHeight; j++) { // Rows (Z axis)
    for (let i = 0; i < gridWidth; i++) { // Columns (X axis)
  ...
  if (startTile) {
    ...
    character.mesh.position.set(startTile.position.x, startTile.position.y + 1.5, startTile.position.z)
  ...
  camera.position.copy(gridCenter).add(new THREE.Vector3(cameraDistance, cameraDistance, cameraDistance))
...
function toggleSelect(mesh: THREE.Mesh) {
  ...
  if(mesh.name === "Character") {
   ...
   let neighbors = getNeighbors(characters[0].tile.gridX, characters[0].tile.gridY,15);
...
function move(mesh: THREE.Mesh) {
  ...
  if (coords) {
    ...
    let neigthbors = getNeighbors(character.tile.gridX, character.tile.gridY,15);
    ...
    if (targetTile) {
      ...
      character.mesh.position.set(targetTile.position.x, targetTile.position.y + 1.5, targetTile.position.z)
...
function dispose() {
  ...
  scene.traverse((object: THREE.Object3D) => { // <-- Type 'object' here
    ...
    if (object instanceof THREE.Mesh) {
      ...
      if (object.material) {
        if (Array.isArray(object.material)) {
          ...
          object.material.forEach((material: THREE.Material) => { // <-- Type 'material' here
            material.dispose()
          })
        } else {
...
<template>
  <div
    ref="box"
    style="width: 100dvw; height: 100dvh; background-color: #f3f4f6; cursor: grab;"
  />
  <CharacterInfo v-if="selected" :character="{ name: 'Hexagon', health: 100, level: 1 }" />
</template>